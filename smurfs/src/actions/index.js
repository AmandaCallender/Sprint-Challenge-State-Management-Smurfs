import axios from 'axios';
 ...
 export const DATA_POST_SUCCESS = 'DATA_GET_SUCCESS';
 ...
 export const getData = () => dispatch => {
   ...
   setTimeout(() => {
     axios
       ...
       .then(response =>
         dispatch({ type: DATA_GET_SUCCESS, payload: response.data })
       )
 ...
 export const postData = addSmurf => dispatch => {
   ...
   axios
     ...
     .then(response => {
       dispatch({ type: DATA_POST_SUCCESS, payload: response.data });
     })
     .catch(errore => {
 ...
 export const deleteSmurf = smurfId => dispatch => {
   ...
   axios.delete(`http://localhost:3333/smurfs/${smurfId}`).then(response => {
     dispatch({ type: DATA_DELETE_SUCCESS, payload: response.data });
   });
 };